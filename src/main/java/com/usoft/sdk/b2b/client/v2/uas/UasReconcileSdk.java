...
public class UasReconcileSdk extends BaseSdk {
	 ...
	 * @return
	 */
	public GetReconcileResp getReconcile(GetReconcileReq.Builder req) throws Exception {
		String url = baseUrl + "/uas/reconcile/get";
		Map<String, String> params = genSignToMap(req);
		String respJson = HttpUtil.doGet(url, params, timeout);
		GetReconcileResp.Builder resp = ProtoBufUtil.toProtoBuf(GetReconcileResp.newBuilder(), respJson);
		return resp.build();
	}
}
